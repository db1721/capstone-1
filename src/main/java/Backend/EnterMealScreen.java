...
/*
  ...
  This class manages the GUI for allowing a user to enter a meal.
*/
...
public class EnterMealScreen extends JFrame {
	...
	// Constructor that sets up the GUI and prompts the user for data
	public EnterMealScreen(MealDatabase mealDatabase, JFrame frame) {
		...
		// Error checking while getting meal data from user
		...
		while (!inputAccepted) {
			try {
				...
				calorieCount = Integer.parseInt(
						JOptionPane.showInputDialog("How many calories does " + "your meal have per serving?"));
				if (calorieCount < 0)
					throw new Exception();

				favoriteFlag = false;
				if (JOptionPane.showConfirmDialog(null, "Is this a favorite meal?", "Favorite?",
						JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
					favoriteFlag = true;
				}

				if (JOptionPane.showConfirmDialog(null, "Allow duplicates of this meal in meal plan?", "Duplicates?",
						JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
					duplicateFlag = true;
				}

				Object mealsOfDay[] = { "Breakfast", "Lunch", "Dinner" };
				mealOfDay = JOptionPane.showInputDialog(null, "What meal of the day is this?", "Meal of Day",
						JOptionPane.QUESTION_MESSAGE, null, mealsOfDay, "Breakfast") + "";

				ingredientSize = Integer
						.parseInt(JOptionPane.showInputDialog("How many" + " ingredients does your meal have?"));
				if (ingredientSize < 1)
					throw new Exception();
				inputAccepted = true;
			} catch (Exception e) {
				JOptionPane.showMessageDialog(null, "Incorrect meal parameters! Please try again!", "IO Error",
		...
		// Creates rows on the panel for each ingredient
		...
		// Returns user to home screen without modifying database
	...
	// Collects ingredient data from each row and creates a meal in the database
	public void addMeal() {
		...
		while (addSuccess) {
			try {
				...
				addSuccess = false;
				JOptionPane.showMessageDialog(null, "Ingredient amounts must be positive numbers!", "IO Error",
						JOptionPane.ERROR_MESSAGE);
			}
			if (addSuccess) {
				for (MealDataRow dataRow : dataRows) {
				...
				mealDatabase.addMeal(mealName, calorieCount, favoriteFlag, duplicateFlag, mealOfDay, ingredientCounts,
						ingredientMeasurements, ingredients);
				JOptionPane.showMessageDialog(null, "Meal successfully entered!", "Success",
