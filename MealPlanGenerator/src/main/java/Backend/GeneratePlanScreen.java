	...
	public void generatePlan(String numberOfMeals, String duplicatePeriod) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Duplicate period and number of days " + "must be integers greater than 0",
					"Incorrect Number Format", JOptionPane.ERROR_MESSAGE);
		}

		// Lets the user know if the meal plan doesn't support the current options
		// selected
		try {
			MealPlan mealPlan = new MealPlan(mealDatabase, mealOptions, numberMeals, duplicateDuration);
